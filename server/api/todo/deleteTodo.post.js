...
export default defineEventHandler(async (e) => {
    ...
    let rows;
    try {
      [rows] = await con.execute(`DELETE \`todo\`
FROM \`todo\`
WHERE 1 AND \`todo\`.\`todo_id\`=:todoId`,
        {todoId: todoId}
        );
    } catch (e) {
      console.log(e);
    ...
    let content = `ユーザ「${user.name}」(${uid})がToDo「」()を削除しました。`;
